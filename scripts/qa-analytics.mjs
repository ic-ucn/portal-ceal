...
try {
  for (const width of [1440, 390]) {
    ...
    const video = page.locator('.portal-reception video');
    await video.evaluate(v => { v.muted = true; });
    await page.locator('[data-reception-play]').click();
    const format = width < 920 ? 'mobile' : 'desktop';
    await hit(`tutorial/${format}/inicio`);
    await video.evaluate(v => { v.pause(); });
    await video.evaluate(v => v.play());
    await page.waitForTimeout(200);
    assert.equal(hits.filter(x => x.p === `tutorial/${format}/inicio`).length, 1, 'resume does not recount video start');
    await video.evaluate(v => { v.currentTime = v.duration * .76; });
    await hit(`tutorial/${format}/75`);
    await video.evaluate(v => { v.currentTime = v.duration - .2; });
    await hit(`tutorial/${format}/fin`);
